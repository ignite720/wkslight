...
int AppCoreWeb::init(int width, int height, bool linear_filter) {
    ...
    do {
    ...
    
        {
            ...
            m_resource_bundle->put(ResourceBundle::AUDIO_CLIP_NEW_RECORD, std::make_unique<AudioClip>("assets/sounds/new record.wav"));
...
void AppCoreWeb::render() {
    const auto lam_draw_text = [=](const String &text, float &y, float line_spacing = 3.0f) {
        int w = 0, h = 0;
        //m_resource_bundle->text_size(ResourceBundle::FONT_PRESS_START_2P, text, &w, &h);
