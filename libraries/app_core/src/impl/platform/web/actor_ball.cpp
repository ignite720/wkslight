...
void Ball::update(float dt) {
    ...
    if (!this->get_dead()) {
        m_rotation = std::fmodf(m_rotation + ROTATION_SPEED * dt, 360.0f);
...
bool Ball::update_collision(float dt, const SDL_FRect *paddle_rect) {
    const auto lam_calculate_velocity_scale = [=](float scale) {
        return std::log10f(this->app_core_as_mut_ptr()->app_info_as_ref().game_info.stats.score + 1.0f) * scale + 1.0f;
    };
    ...
    const auto rect_for_test = this->rect_predicted(2);
    if (!this->get_dead() && SDL_HasIntersectionF(&rect_for_test, paddle_rect)) {
        {
            ...
            if (this->app_core_as_ptr()->app_info_as_ref().game_info.stats.score > this->app_core_as_ptr()->app_info_as_ref().gamesave.high_score) {
                ...
                utils::web::web_fetch_persist_file_store(consts::text::GAMESAVE_FILE_NAME, &gamesave, sizeof(gamesave));
